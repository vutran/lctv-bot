...
export default class Client {
  ...
  handleStanza(stanza) {
    ...
    if (stanza.getChild('delay') === undefined) {
      ...
      // check for @mentions (username, @username)
      let mentions = new RegExp("\@?" + this.getUsername()).test(stanza.getChildText('body'))
      if (mentions) {
        // retrieve the username and strip the room
        let username = stanza.getAttr('from').replace(this.getRoom() + '/', '')
        // if the mention matches your username
