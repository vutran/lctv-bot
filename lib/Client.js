...
export default class Client {
  ...
  handleStanza(stanza) {
    ...
    if (stanza.getChild('delay') === undefined) {
      // check for commands
      let cmds = new RegExp(/^\![\w\d_-]+/).exec(stanza.getChildText('body'))
      if (cmds) {
        // strip leading "!"
        let cmd = cmds[0].substr(1)
        // emits the command event
        this.emit('lctv:cmd', cmd, stanza)
        // if the user is admin
        if (stanza.getAttr('from').replace(this.getRoom() + '/', '') === this.getUsername()) {
          this.emit('lctv:cmd:admin', cmd, stanza)
        }
      }
    }
  }
   ...
   */
  handleAdminCommands(cmd, stanza) {
  ...
  handleNewPresence(stanza) {
    if (stanza.getAttr('type') !== "unavailable") {
      ...
      if (user !== this.getUsername()) {
        this.say('Welcome ' + user + ' to the stream.')
        // if not yet in the users list
        if (!this.users.exists(user)) {
          // add user to the users list
          this.users.add(user)
